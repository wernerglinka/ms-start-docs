...
const docs = (function() {
  ...
  function updateDeepObject(deepObject, flatValues) {
    ...
    function deleteProps(obj, props) {
      /**
       Make sure props are sorted in a way that deeper nested properties are deleted first, especially for arrays. This ensures that when deleting and reindexing array elements, it doesn’t interfere with the deletion of other properties. For example, if you need to delete both arr[2] and arr[3], delete arr[3] first; otherwise, after deleting arr[2], arr[3] will become arr[2].
       */
      props.sort((a, b) => {
        const getIndex = (str) => parseInt(str.match(/\[(\d+)\]/)[1]);
        return getIndex(b) - getIndex(a);
      });

      for (let prop of props) {
        let keys = prop.split(/\.|\[|\]/).filter(Boolean); // Splits by . or [ or ] and filters out empty strings

        let lastKey = keys.pop();
        if(/^\d+$/.test(lastKey)) lastKey = parseInt(lastKey); // Converts lastKey to integer if it's a string number
        let temp = obj;
        for (let key of keys) {
          if(temp[key] !== undefined) temp = temp[key];
        }
        if(Array.isArray(temp)) {
          temp.splice(lastKey, 1); // Removes the element at lastKey and reindexes the array
        } else {
          delete temp[lastKey];
        }
      }
