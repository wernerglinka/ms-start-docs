...
const docs = (function() {
  ...
  function updateDeepObject(deepObject, flatValues) {
    ...
    let flatDeepObject = flattenObject(deepObject);
    let propsToDelete = Object.keys(flatDeepObject).filter(prop => !flatValues.hasOwnProperty(prop));
    
    return deleteProps(deepObject, propsToDelete);
  }
  ...
  const watchPropsFormUpdateSectionDisplay = function (templateName) {
    ...
    propsForm.addEventListener('submit', e => {
      ...
      //console.log(`section after update: ${JSON.stringify(section, null, 2)}`);
